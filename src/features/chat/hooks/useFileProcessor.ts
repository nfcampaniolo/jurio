...
export const useFileProcessor = ({ user, setIsProcessingFiles, setAttachedDocs, setArchiveDocs, setDenyOpen }: FileProcessorProps) => {
  ...
  const processFilesParallel = async (files: File[], promptId: string, targetFascicoloId?: string) => {
    ...
    try {
      ...
      const { loadMaxima, checkDuplicateDocument } = await import("@/shared/services/document");
      const { loadSentence } = await import("@/shared/services/storage");
      const { extractTextFromMedia, extractTextFromFile } = await import("@/shared/services/extractors"); 
