import { makeRiferimentiNormativiKeys } from '@/shared/hooks/riferimentiTranslator'; 
...
export async function cercaPrecedentiPerNorme(norme: string[], maxResults: number = 41) {
  ...
  try {
    ...
    const { getDb } = await import('@/infrastructure/db'); // Assicurati che il path sia corretto
    const db = await getDb();
