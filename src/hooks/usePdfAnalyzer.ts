...
import { extractTextFromFile, getFileType, type DocumentFileType, ocrFileWithTesseract, extractTextFromMedia } from "@/hooks/extractors";  
...
export const usePdfAnalyzer = () => {
  ...
  const analyzeReason = async (promptId: string) => {
    ...
    try {
      if (file && file.name) {
        const { checkDuplicateDocument } = await import("@/services/document");
  ...
  const executeAnalysis = async (promptId?:string, overwriteId?: string, ) => {
    if (!extractedText || !user) return;
    const { loadMaxima } = await import("@/services/document");
    ...
    try {
      ...
      if (overwriteId) {
        try {
          const { deleteDocument } = await import("@/services/document"); 
